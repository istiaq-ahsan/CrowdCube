...
async function run() {
  try {
    ...
    const cfDonatedCollection = client
      .db("cfDonatedDB")
    ...
    app.get("/project", async (req, res) => {
      ...
      // Apply sorting only if sortOrder is defined
      const result = await cfProjectCollection
        .find({})
        .sort(sortOrder ? { amount: sortOrder } : {}) // Use .sort() for sorting
        .toArray();
    ...
    app.put("/project/:id", async (req, res) => {
      ...
      const result = await cfProjectCollection.updateOne(
        filter,
        updateDoc,
        options
      );
